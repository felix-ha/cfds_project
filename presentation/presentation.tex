...
\ \\
 ...
 \item Important: Fit on the training set and then apply imputation on the validation and test set. The same 
...
\frame{\frametitle{Split of the dataset}
 \begin{itemize}
 \item model selection / assesmsent   
...
\begin{itemize}
\item Machine Learning Models can outperfrom approaches!
\item At least in the given evaluation framework presented
\item Deep Leraning is not good when there is not that much data
\item Even simple Machine Learning Model have a decent performance.
\item Data is very important. 
\item Data Colleciton and Handling take the most time of the project budget, modelling takes only a fraction.  

\item SVR failed on training with all countries
\end{itemize}
